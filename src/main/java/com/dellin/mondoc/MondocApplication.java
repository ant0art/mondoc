...
public class MondocApplication {
	...
	public CommandLineRunner run(UserService userService, RoleService roleService) {
		return args -> {
			...
			userService.create(
					new UserDTO("john@travolta.com", "1234", new ArrayList<>()));
			userService.create(new UserDTO("will@Smith.com", "1234", new ArrayList<>()));
			userService.create(new UserDTO("jim@carry.com", "1234", new ArrayList<>()));
			userService.create(new UserDTO("sarah@conor.com", "1234", new ArrayList<>()));
